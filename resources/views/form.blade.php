		...
		<div class="container-contact100">
			<div class="wrap-contact100">
				...
				<form class="contact100-form validate-form" id="e-recruit" method="POST" action="{{ route('submit') }}" enctype="multipart/form-data">
					...
					@endif
					@if (session('alert'))
					<div class="alert alert-danger" style="height:20px; font-size:12px; line-height:2px;">
							<button type="button" style="line-height:2px;" class="close" data-dismiss="alert">&times;</button>
							{!! session('alert') !!}
					</div>
					@endif
					@if ($errors->any())
					...
					<div class="wrap-input100 validate-input">
						<input id="jenis" class="input100" type="text" name="jenis" value="JOB" style="color: #BDBDBD" readonly>
						<span class="focus-input100"></span>
						<label class="label-input100" for="name">
					...
					<div class="wrap-input100 validate-input"> 
						<select name="job" id="name" class="input100" style="height: 30px; color: #BDBDBD" value="{{ old('job') }}" required>
							...
							@foreach ($job as $j)
								@if(date_create() < date_create($j->tanggal))
								<option value="{{$j->judul}}">{{$j->judul}}</option>
								@endif
							@endforeach
